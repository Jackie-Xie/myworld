...
$(function () {
	...
	$( '#top' ).css( {
		paddingLeft:( $( window ).width() - $( '#top' ).width() ) / 2 
	} );
	var paper = $( '.paper' );
	var FW = $( window ).width();
	var FH = $( '#main' ).height();
	...
	$( '#send-btn' ).click( function(){
		...
		var usernameerr = $('#username-err');
		var emailerr = $('#email-err');
		var contenterr = $('#content-err');
		...
		$.post(handleUrl,{username :username.val(),email :email.val(),content:content.val(),},function (data){
			if(data.status){
